...
const TimelineItem = ({ 
    ...
    return (
        ...
        >
            ...
            <div className="z-20 flex items-center order-1 bg-blue-600 shadow-xl w-10 h-10 rounded-full justify-center">
                {type === 'edu' ? <GraduationCap className="text-white w-5 h-5" /> : <Briefcase className="text-white w-5 h-5" />}
            </div>
            
            <div className={`order-1 w-5/12 px-6 py-4 rounded-xl shadow-md border border-slate-100 bg-white ${isEven ? 'text-right' : 'text-left'}`}>
                <p className="mb-1 text-sm font-semibold text-blue-600 tracking-wide uppercase">{date}</p>
                <h3 className="mb-1 font-bold text-slate-800 text-lg">{title}</h3>
                <h4 className="mb-2 text-sm text-slate-500 font-medium">{subtitle}</h4>
                {extra && <p className="text-sm text-slate-600 bg-slate-50 inline-block px-2 py-1 rounded">{extra}</p>}
            </div>
        </motion.div>
    );
};
        ...
        <section className="py-20 bg-white px-6 overflow-hidden">
            <div className="max-w-5xl mx-auto">
                <div className="text-center mb-16">
                    <h2 className="text-3xl font-bold text-slate-900 mb-4">Journey So Far</h2>
                    <p className="text-slate-500">My academic background and professional internships.</p>
                </div>
                    ...
                    <div className="lg:hidden space-y-8">
                        {data.internships.map((job, idx) => (
                         ...
                         {data.education.map((edu, idx) => (
                            <div key={`mob-edu-${idx}`} className="bg-white p-6 rounded-xl shadow-md border-l-4 border-emerald-500">
                                <span className="text-xs font-bold text-emerald-600 uppercase tracking-wider">{edu.duration}</span>
                                <h3 className="text-lg font-bold text-slate-900 mt-1">{edu.degree}</h3>
                                <p className="text-slate-600">{edu.institution}</p>
                                <p className="text-sm text-slate-500 mt-2">{edu.score}</p>
                            </div>
                        ))}
                    </div>
